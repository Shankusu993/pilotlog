...
class FlightImporter(BaseImporter):
    ...
    def validate(self):
        """Validates the flight record."""
        if not self.meta.get("DateUTC"):
            raise ValueError("Flight 'DateUTC' is required.")
        if not self.meta.get("AircraftCode"):
            raise ValueError("Flight 'AircraftCode' is required.")
        return True
    ...
    def process(self):
        ...
        if self.validate():
            aircraft = Aircraft.objects.filter(
                guid=self.meta.get("AircraftCode")
            ).first()
            departure_airfield = Airfield.objects.filter(
                guid=self.meta.get("DepCode")
            ).first()
            arrival_airfield = Airfield.objects.filter(
                guid=self.meta.get("ArrCode")
            ).first()
